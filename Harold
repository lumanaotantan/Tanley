module.exports.config = {
  ...
  version: "1.0.0",
  hasPermission: 0,
  description: "Tanley bot that can download facebook videos, sing, search image, search lyrics and more..",
  usePrefix: false,
  credits: "labi ni raiza and Modified by Tantan(chatbot)",
  usages: "[help]",
  commandCategory: "Tanley",
  cooldowns: 10,
};
...
const { threadID, messageID, body } = event;
  ...
  if (txt.toLowerCase() == "help"){
    const message = `•——[TANLEY BOT]——•
THESE ARE THE THINGS I CAN DO AS A BOT.

•Tanley CAN SING SONGS.
...
if (!txt) return api.sendMessage(rand1, threadID, messageID);
 if (txt.toLowerCase().startsWith("thank you") || txt.toLowerCase().startsWith("thank")) return api.sendMessage("You're welcome user 🤗", threadID, messageID);
  ...
  if (txt.toLowerCase().includes("who created you") || txt.toLowerCase().includes("who's your master") || txt.toLowerCase().includes("who create you") || txt.toLowerCase().includes("who's your creator")) return api.sendMessage("I'm Harold Hutchins Modified by Tantan and original this code by tanley.", event.threadID, event.messageID);
  if (txt.toLowerCase().includes("who is Tantan") || txt.toLowerCase().includes("who is Tantan")) return api.sendMessage("My Bot Admin", threadID, messageID);
